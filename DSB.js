...
// ============================================================
// ⚙️ ACCOUNT SETTINGS — Change Password / Profile Info / Delete
// Account. newClientModal jaisa hi split: DSBstyle.js modal ke
// open/close aur element-selectors rakhta hai, yeh file sirf apne
// alag listeners attach karti hai (local re-query se, DSBstyle.js
// ke top-level const pe depend kiye bina) jahan bhi Firebase touch
// hota hai.
// ============================================================
...
document.getElementById("openSettingsBtn")?.addEventListener("click", () => {
    ...
    db.collection("users").doc(user.uid).get().then((doc) => {
        if (!doc.exists) return;
        const data = doc.data();
        const businessNameInput = document.getElementById("settingsBusinessNameInput");
        const websiteInput = document.getElementById("settingsWebsiteInput");
        if (businessNameInput) businessNameInput.value = data.businessName || "";
...
document.getElementById("saveProfileBtn")?.addEventListener("click", async () => {
    ...
    const businessName = document.getElementById("settingsBusinessNameInput")?.value.trim() || "";
    const websiteUrl = document.getElementById("settingsWebsiteInput")?.value.trim() || "";
